package org.as3commons.logging.setup.target {
	...
	public class FrameBufferTest extends MockitoTestCase {
		...
		public function testBuffer(): void {
			...
			buffer.log( "name", "shortName", DEBUG, 12345, "Sample", arr1, null, null, null);
			buffer.log( "name2", "shortName", DEBUG, 12467, "Test", _arr2, null, null, null);
			buffer.log( "hose", "shortName", DEBUG, 12467, "Demo", _arr2, null, null, null);
			
			inOrder().verify().that( _targetMock.log( eq("name"), eq("shortName"), eq(DEBUG), eq(12345), eq("Sample"), eq(arr1), eq(null), eq(null), eq(null) ) );
			inOrder().verify().that( _targetMock.log( eq("name2"), eq("shortName"), eq(DEBUG), eq(12467), eq("Test"), eq(_arr2), eq(null), eq(null), eq(null) ) );
